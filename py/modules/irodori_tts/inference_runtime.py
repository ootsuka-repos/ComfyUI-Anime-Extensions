...
def offload_cached_runtime() -> None:
    """Compatibility hook for the ComfyUI node cache policy.

    The v4 pretrained backbone is not safely movable piecemeal across all supported
    torch/transformers versions. Releasing the cached runtime is therefore the
    reliable low-VRAM equivalent of the former CPU-offload policy.
    """
